...
-   **Atualização da Estrutura do Projeto (`tree.md`):** O arquivo `tree.md` documenta a estrutura de pastas e arquivos do seu projeto, excluindo itens ignorados pelo `.gitignore`. Para manter este documento sempre atualizado, você deve:
    1.  **Executar o script `tree.ps1`:** Abra seu terminal PowerShell na raiz do projeto e execute `.\tree.ps1`. Este script irá gerar uma lista de arquivos que eu usarei.
    2.  **Pedir para eu processar a árvore:** Após executar o `tree.ps1`, me diga "Atualize o tree.md". Eu lerei a lista gerada, processarei-a para construir a árvore formatada e sobrescreverei o `tree.md` com a nova estrutura.
